<x-app-layout>
    <x-page-header title="Transfer Approvals" subtitle="Approve or reject outgoing transfers from your department.">
    </x-page-header>
    ...
    <x-bento-card :padded="false">
        @if($pending->isEmpty())
            <x-empty-state icon="check-circle" title="No pending transfers" hint="All outgoing transfers have been reviewed."/>
        @else
            <x-table :headers="['Transfer #', 'Requested By', 'Destination', 'Items', 'Purpose', 'Date', 'Actions']">
                @foreach($pending as $t)
                    <x-table.row>
                        ...
                        <td class="px-6 py-3 text-sm text-ink-body">{{ $t->requestedBy->name }}</td>
                        <td class="px-6 py-3 text-sm text-ink-body">{{ $t->toDept->name }}</td>
                        <td class="px-6 py-3 text-sm text-ink-body">{{ $t->items->count() }}</td>
                        <td class="px-6 py-3 text-sm text-ink-muted max-w-xs truncate">{{ $t->purpose }}</td>
                        ...
                        <td class="px-6 py-3">
                            <div class="flex items-center gap-2" x-data="{ rejecting: false }">
                                ...
                                <div x-show="rejecting" x-transition class="absolute z-10 mt-2 bg-surface-tile border border-surface-border rounded-lg p-3 shadow-lg w-64">
                                    <form method="POST" action="{{ route('transfers.reject', $t) }}" class="space-y-2">
                                        @csrf @method('PATCH')
